...
use crate::binaries::{resolve_cli_path, resolve_bundled_tool_path};
use crate::env::{get_proxy_auth_token, get_proxy_url, load_env_fallback};
use crate::ipc::parse_and_emit;
use crate::server::{find_video_file, serve_beats_json, serve_viewer_html, serve_video};
use crate::state::AppState;
use crate::types::{
    BeatsFile, BeatsResponse, DoneMsg, ProjectInfo, TopicEntry,
};
...
async fn start_pipeline(
    ...
    eprintln!(
        "[videotool-app] proxy_url present: {}, compile-time present: {}, runtime present: {}",
        proxy_url.is_some(),
    ...
    if let Some(url) = proxy_url {
        ...
        eprintln!(
            "[videotool-app] forwarding VITE_API_PROXY_URL (len={})",
            url.len()
        );
    }
            ...
            match child.wait().await {
                ...
                Err(e) if !got_terminal => {
                    let _ = app_clone.emit(
                        "pipeline-error",
                        format!("Failed to reap pipeline process: {e}"),
                    );
                }
                _ => {}
            }
