"""Input validation utilities for vodtool."""
...
def validate_project_path(project_path: Path) -> Optional[str]:
    """
    Validate that a path points to a valid vodtool project.
    ...
    if not meta_path.exists():
        return f"Not a valid vodtool project (missing meta.json): {project_path}"
...
def get_projects_dir() -> Path:
    """
    Get the vodtool projects directory.

    Returns ~/.vodtool/projects, creating it if needed.

    Returns:
        Path to projects directory
    """
    projects_dir = Path.home() / ".vodtool" / "projects"
