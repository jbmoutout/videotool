"""File handling utilities for vodtool."""
...
def project_lock(project_path: Path, *, timeout: int = 5):
    """
    Acquire an exclusive lock on a vodtool project to prevent concurrent access.
    ...
    """
    lock_file = project_path / ".vodtool.lock"
    lock_fd = None
    ...
    try:
        ...
        try:
            ...
            console.print(
                f"[yellow]Warning: Project is locked (another vodtool process is using it)[/yellow]"
            )
            ...
            try:
                ...
                console.print("[dim]Wait for other process to finish, or remove .vodtool.lock file[/dim]")
