"""Transcription command for videotool."""
...
from videotool.utils.file_utils import project_lock, safe_write_json
...
def transcribe_audio(
    ...
    if not audio_path.exists():
        ...
        console.print("Run 'videotool ingest' first to create a project with audio.")
        return None
    ...
    try:
        if provider == "groq":
            from videotool.transcription import GroqTranscriptionProvider
            transcription_provider = GroqTranscriptionProvider(
                model=model_name or "whisper-large-v3-turbo"
            )
        elif provider == "openai":
            from videotool.transcription import OpenAITranscriptionProvider
