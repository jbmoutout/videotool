...
def diarize_review_command(project_path: Path = typer.Argument(..., help="Path to project folder")):
    ...
    if not diarization_file.exists():
        ...
        console.print("Run 'videotool diarize' first to generate diarization data.")
        raise typer.Exit(1)

    if not speaker_map_file.exists():
        console.print(f"[red]Error: speaker_map.json not found in {project_path}[/red]")
        console.print("Run 'videotool diarize' first to generate speaker map.")
        raise typer.Exit(1)
    ...
    console.print("  1. Re-run 'videotool chunks' to update chunk speaker labels")
    console.print("  2. Re-run 'videotool embed' if needed (to update database)")
    console.print("  3. Continue with 'videotool segment-topics' and 'videotool topics'")
